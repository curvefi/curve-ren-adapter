...
interface ICurveExchange {
    ...
    function calc_token_amount(uint256[__N_COINS__] calldata amounts, bool deposit) external returns (uint256 amount);

    function add_liquidity(uint256[__N_COINS__] calldata amounts, uint256 min_mint_amount) external;

    function remove_liquidity(
        uint256 _amount,
        uint256[__N_COINS__] calldata min_amounts
    ) external;

    function remove_liquidity_imbalance(uint256[__N_COINS__] calldata amounts, uint256 max_burn_amount) external;
...
contract CurveExchangeAdapter is GSNRecipient {
    ...
    IERC20[__N_COINS__] coins;
    uint256[__N_COINS__] precisions_normalized = [1,1,1e10];
    ...
    event DepositMintedCurve(uint256 mintedAmount, uint256 curveAmount, uint256[__N_COINS__] amounts);
    event ReceiveRen(uint256 renAmount);
    event Burn(uint256 burnAmount);

    constructor(ICurveExchange _exchange, address _curveTokenAddress, IGatewayRegistry _registry, IERC20[__N_COINS__] memory _coins) public {
        exchange = _exchange;
        registry = _registry;
    ...
    function mintThenDeposit(
        ...
        uint256 _amount, 
        uint256[__N_COINS__] calldata _amounts, 
        ...
        uint256[__N_COINS__] memory receivedAmounts = _amounts;
    ...
    function removeLiquidityThenBurn(bytes calldata _btcDestination, uint256 amount, uint256[__N_COINS__] calldata min_amounts) external discountCHI {
        uint256[__N_COINS__] memory balances;
    ...
    function removeLiquidityImbalanceThenBurn(bytes calldata _btcDestination, uint256[__N_COINS__] calldata amounts, uint256 max_burn_amount) external discountCHI {
        uint256[__N_COINS__] memory balances;
        for(uint256 i = 0; i < coins.length; i++) {
            balances[i] = coins[i].balanceOf(address(this));
        }
